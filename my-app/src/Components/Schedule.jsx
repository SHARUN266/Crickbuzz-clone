 ...
 //console.log(list, "match scheduelee");
  return (
    <Box w="100%" m="auto" textAlign={"justify"} p="5rem">
      <Heading ml="1%" display={'flex'} gap="1rem">Cricket Schedule <Image h="40px" mt="0.5%" src="https://t4.ftcdn.net/jpg/01/18/06/01/240_F_118060147_9V9FMQeC9FHtT8dfjsSnzyhZTOYaDYcC.jpg"/> </Heading>
  ...
  <Table mt="5%"   size='md'>
    ...
    {
       list.length==0?'Loading': list.map((elem)=>(
    <Tbody>
  
      <Tr _hover={{bg:"groupHover.bg",color:"black"}} cursor='pointer'>
     
        <Td key={elem.id} fontWeight="700" >{elem.date}</Td>
        <Td>{elem.name}</Td>
        <Td >{elem.venue}</Td>
        <Td>{elem.status}</Td>
          </Tr>
